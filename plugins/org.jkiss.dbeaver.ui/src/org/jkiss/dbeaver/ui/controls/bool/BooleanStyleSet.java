...
import org.jkiss.dbeaver.ui.UIElementFont;
...
public class BooleanStyleSet {
    ...
    private static BooleanStyle getDefaultStyle(@NotNull DBPPreferenceStore store, @NotNull BooleanMode mode, @NotNull BooleanState state, @NotNull RGB defaultColor) {
        ...
        if (mode == BooleanMode.TEXT) {
            ...
            final UIElementFont font = CommonUtils.valueOf(UIElementFont.class, store.getString(namespace + PROP_FONT), UIElementFont.NORMAL);
    ...
    private static BooleanStyle getDefaultStyleLegacy(@NotNull DBPPreferenceStore store, @NotNull BooleanState state, @NotNull RGB color) {
        switch (store.getString(PREF_BOOLEAN_STYLE)) {
            ...
            case PROP_LEGACY_STYLE_CHECKBOX:
                return BooleanStyle.usingText(state.choose("☑", "☐", "☒"), UIElementAlignment.CENTER, color, UIElementFont.NORMAL);
            case PROP_LEGACY_STYLE_TRUE_FALSE:
                return BooleanStyle.usingText(state.choose("true", "false", DBConstants.NULL_VALUE_LABEL), UIElementAlignment.CENTER, color, UIElementFont.NORMAL);
            case PROP_LEGACY_STYLE_YES_NO:
                return BooleanStyle.usingText(state.choose("yes", "no", DBConstants.NULL_VALUE_LABEL), UIElementAlignment.CENTER, color, UIElementFont.NORMAL);
            case PROP_LEGACY_STYLE_TEXTBOX:
            default:
                return BooleanStyle.usingText(state.choose("[v]", "[ ]", DBConstants.NULL_VALUE_LABEL), UIElementAlignment.CENTER, color, UIElementFont.NORMAL);
        }
    }
    ...
    private static void setDefaultStyle(@NotNull DBPPreferenceStore store, @NotNull BooleanStyle style, @NotNull BooleanState state, @NotNull RGB defaultColor) {
        ...
        if (style.getMode() == BooleanMode.TEXT) {
            ...
            store.setValue(namespace + PROP_FONT, style.getFont().name());
